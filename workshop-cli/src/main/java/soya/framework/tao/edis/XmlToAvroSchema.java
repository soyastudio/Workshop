...
public class XmlToAvroSchema {
    private static Set<Schema> records = new HashSet<>();

    private XmlToAvroSchema() {
    }

    public static Schema fromXmlSchema(SchemaTypeSystem sts) {
        return fromXmlSchema(sts.globalTypes()[0]);
    }
    ...
    public static Schema fromXmlSchema(SchemaType schemaType) {
        ...
        SchemaBuilder.FieldAssembler assembler = SchemaBuilder
                .record(schemaType.getName().getLocalPart()).namespace(schemaType.getName().getNamespaceURI())
                .fields();

        for (SchemaProperty ap : schemaType.getAttributeProperties()) {
            assembleAttribute(ap, assembler);
        }
    ...
    private static SchemaBuilder.FieldAssembler assemble(SchemaProperty sp, SchemaBuilder.FieldAssembler assembler) {
        ...
        if (st.isSimpleType()) {
            ...
            QName name = st.isAnonymousType() ? sp.getName() : sp.getType().getName();
            if (sp.getMaxOccurs() == null || sp.getMaxOccurs().intValue() > 1) {
                assembler.name(name.getLocalPart()).type().array().items().type(assembleComplexProperty(sp));

            } else {
                assembler.name(name.getLocalPart()).type(assembleComplexProperty(sp)).noDefault();
    ...
    private static Schema assembleComplexProperty(SchemaProperty sp) {
        SchemaType schemaType = sp.getType();
        SchemaBuilder.FieldAssembler assembler;
        Set<QName> names = new HashSet<>();

        if (schemaType.isAnonymousType()) {
            assembler = SchemaBuilder
                    .record(sp.getName().getLocalPart()).fields();
        } else {
            assembler = SchemaBuilder
                    .record(sp.getType().getName().getLocalPart()).namespace(sp.getType().getName().getNamespaceURI()).fields();
        }
        ...
        for (SchemaProperty ppt : schemaType.getElementProperties()) {
            assemble(ppt, assembler);
        }
    ...
    private static void assembleAttribute(SchemaProperty sp, SchemaBuilder.FieldAssembler assembler) {
        String defaultText = sp.getDefaultText();
        ...
        if (sp.getMaxOccurs() == null || sp.getMinOccurs().intValue() == 0) {
