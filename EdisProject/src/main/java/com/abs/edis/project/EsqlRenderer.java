...
public abstract class EsqlRenderer extends EdisRenderer {
    ...
    protected Stack<EdisTask.Array> arrayStack = new Stack<>();
    ...
    protected void printNode(KnowledgeTreeNode<XsNode> node, StringBuilder builder, int indent) {
        if (node.getAnnotation(NAMESPACE_ASSIGNMENT) != null) {
            ...
            EdisTask.Function[] functions = construction.getFunctionArray();
            ...
            if(len == 0) {
                ...
                EdisTask.Function function = construction.getFunctionArray()[0];
                String arrName = function.parameters[0];
                Iterator<EdisTask.Array> iterator = arrayStack.iterator();
    ...
    protected void printLoopFunction(EdisTask.Function function, KnowledgeTreeNode<XsNode> node, StringBuilder builder, int indent) {

        EdisTask.Construction parentConstruction = ((KnowledgeTreeNode<XsNode>) node.getParent()).getAnnotation(NAMESPACE_CONSTRUCTION, EdisTask.Construction.class);
        EdisTask.Construction construction = node.getAnnotation(NAMESPACE_CONSTRUCTION, EdisTask.Construction.class);
    ...
    protected void printArrayFolder(KnowledgeTreeNode<XsNode> node, StringBuilder builder, int indent) {
        ...
        for (EdisTask.Function e : construction.getFunctions()) {
            if (FUNCTION_ARRAY.equals(e.name) && e.parameters.length == 3) {

                EdisTask.Array array = new EdisTask.Array();
    ...
    protected void printAssignment(KnowledgeTreeNode<XsNode> node, StringBuilder builder, int indent) {
        ...
        EdisTask.Construction construction = ((KnowledgeTreeNode<XsNode>) node.getParent()).getAnnotation(NAMESPACE_CONSTRUCTION, EdisTask.Construction.class);

        //String assignment = getAssignment(mapping, inputRootVariable);
        if (assignment != null && assignment.getFirst() != null) {
            EdisTask.Function function = assignment.getFirst();
            boolean print = false;
    ...
    protected String getAssignment(EdisTask.Function function) {
